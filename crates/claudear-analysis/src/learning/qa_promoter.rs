...
use claudear_core::types::PromotedInstruction;
...
impl QaPromoter {
    ...
    pub fn scan_and_promote(
        ...
        let mut groups: std::collections::HashMap<
            (String, String),
            Vec<&claudear_core::types::QaMatch>,
    ...
    fn check_embedding_similarity(
        entries: &[&claudear_core::types::QaMatch],
        threshold: f64,
    ) -> bool {
    ...
    fn compute_confidence(entries: &[&claudear_core::types::QaMatch]) -> f64 {
...
mod tests {
    ...
    fn test_compute_confidence() {
        use claudear_core::types::{QaKnowledgeEntry, QaMatch};
    ...
    fn test_scan_and_promote_with_qa_entries() {
        ...
        for i in 0..3 {
            let entry = claudear_core::types::QaKnowledgeEntry {
                id: 0,
                source: "linear".to_string(),
                repo: Some("org/repo".to_string()),
    ...
    fn make_qa_match(success_rate: f64) -> claudear_core::types::QaMatch {
        claudear_core::types::QaMatch {
            entry: claudear_core::types::QaKnowledgeEntry {
                id: 0,
                source: "linear".to_string(),
                repo: Some("org/repo".to_string()),
                issue_id: "iss".to_string(),
    ...
    fn test_compute_confidence_empty_entries() {
        ...
        let entries: Vec<claudear_core::types::QaMatch> = vec![];
        let refs: Vec<&claudear_core::types::QaMatch> = entries.iter().collect();
        let confidence = QaPromoter::compute_confidence(&refs);
        assert!(
    ...
    fn test_compute_confidence_above_count_cap() {
        ...
        let entries: Vec<claudear_core::types::QaMatch> =
            (0..10).map(|_| make_qa_match(0.5)).collect();
        let refs: Vec<&claudear_core::types::QaMatch> = entries.iter().collect();
        let confidence = QaPromoter::compute_confidence(&refs);
        assert!(
    ...
    fn test_compute_confidence_all_zero_success() {
        ...
        let entries: Vec<claudear_core::types::QaMatch> =
            (0..3).map(|_| make_qa_match(0.0)).collect();
        let refs: Vec<&claudear_core::types::QaMatch> = entries.iter().collect();
        let confidence = QaPromoter::compute_confidence(&refs);
        assert!(
