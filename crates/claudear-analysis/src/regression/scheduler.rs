...
use claudear_core::types::{RegressionCheck, RegressionWatch, RegressionWatchStatus};
...
pub struct CheckCycleResult {
    ...
    pub issue_type: claudear_core::types::IssueType,
...
mod tests {
    ...
    impl RegressionChecker for FailingChecker {
        async fn check_regression(&self, _watch: &RegressionWatch) -> Result<RegressionResult> {
            Err(claudear_core::error::Error::api("Sentry API unavailable"))
        }
    }
