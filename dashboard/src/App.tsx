...
import { fetchOverview, getHealth, getRetries, type Overview, type AttemptSummary, type Health, type RetriesResponse } from './lib/api'
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b">
        <div className="container mx-auto py-4 px-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Claudear Dashboard</h1>
            <p className="text-muted-foreground">Monitor automated issue fixing</p>
          </div>
          {health && (
      ...
      <main className="container mx-auto py-6 px-4 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatsCard
            ...
            description="All fix attempts"
          />
          <StatsCard
            title="Success Rate"
            value={`${success_rate.toFixed(1)}%`}
            icon={<CheckCircle2 className="h-4 w-4 text-green-500" />}
            description={`${stats.success + stats.merged} successful`}
          />
          <StatsCard
            title="Merge Rate"
            value={`${merge_rate.toFixed(1)}%`}
            icon={<GitMerge className="h-4 w-4 text-purple-500" />}
            description={`${stats.merged} merged PRs`}
          />
          <StatsCard
            title="Pending"
            value={stats.pending}
...
function SourceRow({ source }: { source: { name: string; total: number; success: number; failed: number; merged: number; success_rate: number } }) {
  return (
    <div className="flex items-center justify-between">
      <div>
...
function AttemptRow({ attempt }: { attempt: AttemptSummary }) {
  ...
  const statusColor = statusColors[attempt.status] || 'bg-gray-100 text-gray-800'
